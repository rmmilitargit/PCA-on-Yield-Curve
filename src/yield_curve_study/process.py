...
def compute_krd_for_bond(maturity: float,
	...
	"""
	pv_base, _, , _ = pv_bond(maturity, coupon_rate, notional, zero_curve_mats, base_zero_rates, freq=freq)
...
def map_krd_to_pc_exposure(krd_matrix: np.ndarray,
	...
	for i in range(n_bonds):
		for k in range(n_components):
			delta_yields = pc_std[k] * pc_components[k] # absolute yield units (e.g., 0.001)
